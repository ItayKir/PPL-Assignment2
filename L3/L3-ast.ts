...
const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> =>{
    if(!isArray(fields) || !allT(isString, fields)){
        return makeFailure(`Expected variables for class field`);
    }
    const classFields = map(makeVarDecl, fields);

    if (!isNonEmptyList<Sexp>(methods) || methods.length > 1) {
        return makeFailure(`Expected exactly one methods list for class`);
    }
    const methodBindings = first(methods);
    if (!isGoodBindings(methodBindings)) {
        return makeFailure(`Malformed methods in "class" expression`);
    }

    const methodNames = map(b => b[0], methodBindings);
    const methodValsResult = mapResult(parseL3CExp, map(second, methodBindings));
    return mapv(methodValsResult, (vals: CExp[]) => 
        makeClassExp(classFields, zipWith(makeBinding, methodNames, vals))
    );
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp) ? `(class (${map((f: VarDecl) => f.var, exp.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, exp.methods).join(" ")}))` :
